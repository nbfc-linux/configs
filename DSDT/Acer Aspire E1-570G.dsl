...
DefinitionBlock ("", "DSDT", 1, "ACRSYS", "ACRPRDCT", 0x00000000)
{
    ...
    OperationRegion (OGNS, SystemMemory, 0x9AFBC618, 0x00000587)
    ...
    Scope (_SB)
    {
        ...
        Device (PCI0)
        {
            ...
            Device (LPCB)
            {
                ...
                Device (MSS0)
                {
                    Name (_HID, EisaId ("SYN1B6D"))  // _HID: Hardware ID
                ...
                Device (MSA2)
                {
                    Name (_HID, EisaId ("AUI2012"))  // _HID: Hardware ID
    ...
    Method (_WAK, 1, Serialized)  // _WAK: Wake
    {
        ...
        If ((Arg0 == 0x04))
        {
            ...
            If (CondRefOf (\_OSI, Local0))
            {
                ...
                {
                    OSYS = 0x07D6
                    \_SB.PCI0.LPCB.EC0.OSTY = One
                }

                If (_OSI ("Windows 2009"))
                {
                    OSYS = 0x07D9
                    \_SB.PCI0.LPCB.EC0.OSTY = 0x03
                }

                If (_OSI ("Windows 2012"))
                {
                    OSYS = 0x07DC
                    \_SB.PCI0.LPCB.EC0.OSTY = 0x04
                }

                If (_OSI ("Linux"))
                {
                    OSYS = 0x03E8
                    \_SB.PCI0.LPCB.EC0.OSTY = 0x02
                }
            }

            If ((\_SB.PCI0.LPCB.EC0.OSTY != Local1))
            {
                Local1 = (\_SB.PCI0.LPCB.EC0.OSTY | 0x80)
                CMSW (0x7A, Local1)
            }

            If (CMSR (0x6F))
            {
                \_SB.PCI0.LPCB.EC0.RFEN = One
            }
        }
                        ...
                        Device (PR25)
                        {
                            ...
                            Name (_PLD, Package (0x01)  // _PLD: Physical Location of Device
                            {
                                ToPLD (
                                    ...
                                    PLD_Rotation           = 0x0,
                                    PLD_Order              = 0x0,
                                    PLD_VerticalOffset     = 0x10B,
                                    PLD_HorizontalOffset   = 0xD0)
                ...
                Device (HSP4)
                {
                    ...
                    Method (_PLD, 0, Serialized)  // _PLD: Physical Location of Device
                    {
                        Name (PLDP, Package (0x01)
                        {
                            ...
                            {
                                /* 0000 */  0x01, 0xC6, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00,  // ..r.....
                                /* 0008 */  0x68, 0x0C, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00   // h.......
                            }
                        })
                        CreateBitField (DerefOf (PLDP [Zero]), 0x40, VIS)
                        If (!(PR2 & 0x08))
